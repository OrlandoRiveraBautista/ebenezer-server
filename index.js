...
// Getting live video --------------------------------------------------------------------------------------
const liveUrl = 'https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=UC1h3zwfeST-fRIrZGGJfJig&maxResults=5&order=date'
app.get('/live', async (req, res) => {
    fetch(`${liveUrl}&key=${process.env.GOOGLE_API_KEY}`)
        ...
        .then( async json => {
            ...
            for( i = 1; i < videoData.length; i++ ){ // initiating going through data
            
                if (videoData[i].snippet.liveBroadcastContent === 'live' ) { // check if any itteration has live video
            ...
            if (liveVidId !== undefined) { // checking if data was assigned
                
                if ( liveVidId !== latestInDB.videoid ) { // checking if the data is already saved
                ...
                if ( liveVidId !== latestInDB.videoid ) { // checking if already in database
                    // Saving the new entry
                    saveVideo.create({
                        videoid: liveVidId,
                        publishedData: publishedDate,
...
app.post('/posts/store', checkAuth, upload.array('file', 1), async (req, res) => {
...
app.post('/event/store', async (req, res) => {
    ...
    if (req.body.title !== undefined && req.body.description !== undefined && req.body.dateStart !== undefined && req.body.dateEnd !== undefined) {
...
app.get('/getUsers', async (req, res) => {
    ...
    await mongoose.connect(uri, { useUnifiedTopology: true, useNewUrlParser: true }).then( async () => {
